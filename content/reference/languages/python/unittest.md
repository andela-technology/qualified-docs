...
Here is a minimal example of unit testing in Qualified's Python 3.7 environment (or more recent) by explicitly importing the `solution` and `preloaded` modules. For [Project Code Challenges](/reference/challenges/multi-file-code), these imports can be any module name in the workspace.
