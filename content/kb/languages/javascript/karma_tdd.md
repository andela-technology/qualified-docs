...
Mocha in Karma works the same as the normal [javascript/mocha_tdd](/kb/languages/javascript/mocha_tdd), with one exception: _Chai_ will already be loaded for you, with `expect`, `should`, and `assert` all available, no `require()` necessary.
...
Please note this feature has been surpassed by [Project Challenges](/kb/hire/challenges/multi-file-code), and will not likely be expanded.
</div>
