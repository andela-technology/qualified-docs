...
export default {
  ...
  data() {
    return {
      expanded: [],
      private:
        process.isClient && window.location.search.indexOf('private') > 0,
    };
  },
  computed: {
    pages() {
    ...
    nestedPages() {
      ...
      return getNestedPages(this.currentPage, this.pages).filter(
        page => page.directory === '/' && !!page.pages.length
      );
    },
  },
  methods: {},
