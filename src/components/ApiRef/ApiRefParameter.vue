<template>
 <div class="api-ref-parameter">
    <div class="api-ref-parameter__name">
        ...
        <span :class="`api-ref-parameter__label api-ref-parameter__label--${optionalLabel}`" v-if="optionalLabel">{{ optionalLabel }}</span>
  ...
  computed: {
      optionalLabel () {
          if (!this.ignoreRequired) {
              return this.parameter.required || this.parameter.name === 'id' ? 'required' : 'optional'
          }
          return null
      },
      typeLabel () {
